...
$(document).ready(function() {
    ...
    // Juges Grid
    var oTableJuges = $('#tableJuges').dataTable({
        "bInfo": false,
    ...
    // Add delete button
    $('#tableJuges_wrapper .row-fluid:last .span6:first').html('<a id="deleteItem" class="btn btn-primary" href="#">Supprimer</a></div>');

    // Add delete button's click handler
    $('#deleteItem').click( function() {
        var anSelected = fcms.fnGetSelected( oTableJuges );
        if ( anSelected.length !== 0 ) {
            var id = oTableJuges.fnGetData(oTableJuges.fnGetPosition(anSelected[0]))[0];
            $.ajax({
                url: modelUrl + id,
                type: 'DELETE',
                complete: function(result) {
                    oTableJuges.fnDeleteRow(anSelected[0]);
    ...
    $("#tableJuges tbody tr").click(function(event) {
        fcms.fnSelectableRows($(this));
    });
    ...
    fcms.fnSelectableRows = function ( row ) {
        ...
        // If item isn't already selected
            ...
            if ($('#tableUsers tr.row_selected')) {
                $('#tableUsers tr.row_selected').removeClass('row_selected');
            }

            if ($('#tableJuges tr.row_selected')) {
                $('#tableJuges tr.row_selected').removeClass('row_selected');
            }
