...
$(document).ready(function() {
    ...
    // Teachers Grid
    var oTableTeachers = $('#tableTeachers').dataTable({
        "bInfo": false,
    ...
    // Add delete button
    $('#tableTeachers_wrapper .row-fluid:last .span6:first').html('<a id="deleteItem" class="btn btn-primary" href="#">Supprimer</a></div>');

    // Add delete button's click handler
    $('#deleteItem').click( function() {
        var anSelected = fcms.fnGetSelected( oTableTeachers );
        if ( anSelected.length !== 0 ) {
            var id = oTableTeachers.fnGetData(oTableTeachers.fnGetPosition(anSelected[0]))[0];
            $.ajax({
                url: modelUrl + id,
                type: 'DELETE',
                complete: function(result) {
                    oTableTeachers.fnDeleteRow(anSelected[0]);
    ...
    $("#tableTeachers tbody tr").click(function(event) {
        fcms.fnSelectableRows($(this));
    });
        ...
        if (formId.val() && userAgreement) {
            ...
            $.ajax({
                ...
                success : function( data ) {
                    ...
                    var iRow = oTableTeachers.fnAddData(aItem);
